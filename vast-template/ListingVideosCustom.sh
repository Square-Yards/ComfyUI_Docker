...
function provisioning_start() {
    ...
    provisioning_setup_listingvideos
...
function provisioning_setup_listingvideos() {
    cd /root/
    git clone https://$GITHUB_API_TOKEN@github.com/Square-Yards/ListingVideos-LS.git
    cd ListingVideos-LS
    git checkout Custom
    conda create -n listing python=3.10 -y
    conda run -n listing pip install -r requirements.txt
...
function provisioning_supervisor_listingvideos() {
    echo '#!/bin/bash
while [ -f "/.provisioning" ]; do
    sleep 5
done
cd /root/ListingVideos-LS
/venv/listing/bin/python main.py
...
autostart=true
autorestart=true
stdout_logfile=/var/log/listingvideos.log
stderr_logfile=/var/log/listingvideos.err.log
